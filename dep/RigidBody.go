...
func (h *RigidBody) Collide(h2 *RigidBody) bool {
	...
	t[1][0], t[1][1] = h2.X+h2.Width, h2.Y
	t[2][0], t[2][1] = h2.X+h2.Width, h2.Y+h2.Height
	t[3][0], t[3][1] = h2.X, h2.Y+h2.Height
	for _, i := range t {
		if h.X <= i[0] && i[0] <= h.X+h.Width && h.Y <= i[1] && i[1] <= h.Y+h.Height {
			return true
		}
	}
	return false
